...
class Level1 extends Phaser.Scene {
	...
	editorCreate() {
		...
		// door
		const door = this.add.rectangle(1224, 730, 64, 350);
		door.isFilled = true;
		...
		// key
		const key = this.add.rectangle(46, 789, 128, 128);
		key.isFilled = true;
	...
	create() {
		...
		ball.label = "ball";
		ground.label = "ground";
		const ballInitInertia = ball.body.inertia


		var mouseSpring = this.matter.add.mouseSpring({});
		var platformMouseStiffness = new Phaser.Physics.Matter.PointerConstraint(this, this.matter.world);
		...
		cons.angleBMin = 0;
		cons.angleBMax = Math.PI * 2 / 3;

		const rampConst1 = this.matter.add.rectangle(639, 730, 128 * 0.1170975530768291, 128 * 0.4837246178719503, {angle: -14 * 0.0174533, isStatic: true, collisionFilter: {group: -5}})
		...
		this.matter.world.on('drag', (body, part, constraint) => {
			if (body.label != 'ball' && body.label != 'ground') {
				if (body.angle >  -120) {
					try {
						body.setAngle(-120);
					} catch {};
		...
		const key = this.matter.add.rectangle(46, 789, 128, 128, {isSensor: true, isStatic: true, render: {fillColor: '#ecd740'}});
		const door = this.matter.add.rectangle(1224, 730, 64, 350, {isStatic: true, collisionFilter: {group :-3}});
