...
class Level1 extends Phaser.Scene {
	...
	editorCreate() {
		...
		// BallObject
		const ballObject = this.add.image(194, 197, "ball");
	...
	create() {
		ball = this.matter.add.sprite(194, 197, "ball", "ball", { shape: this.cache.json.get('collision1').ball });
		ball.setDepth(1);
		ball.setScale(scale)
		...
		this.platForceEnable = platForceEnable;
	...
	update() {
		...
		if (ball.body.velocity.x > this.maxVelocity || ball.body.velocity.x < -this.maxVelocity || null) {
			ball.setVelocityX(ball.body.velocity.x / Math.abs(ball.body.velocity.x) * this.maxVelocity)
		}
		...
		if (this.cValue == 1) {
			if (ball.body.velocity.x > 5) {
				ball.applyForce({ x: -.05 * this.scale, y: 0 });
			}
			ball.applyForce({ x: -.04 * this.scale, y: ball.body.force.y });
			this.cValue = 0;
		}
		if (this.cValue == 2) {
			if (ball.body.velocity.x < -5) {
			...
			ball.applyForce({ x: .04 * this.scale, y: 0 });
			this.cValue = 0;
		}
		if (this.cValue == 3) {
